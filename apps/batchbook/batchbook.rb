module Batchbook
  ...
  class Base < SupportBeeApp::Base
    ...
    def validate
      ...
      test_api_request = http_get api_url('people')
      unless test_api_request.success?
        show_error_notification "Settings Incorrect. Please check the Subdomain and Auth Token"
        return false
      end
    ...
    def required_fields_present?
      ...
      if settings.subdomain.blank?
        show_inline_error :subdomain, "Cannot be blank"
        are_required_fields_present = false
      end
      if settings.auth_token.blank?
        show_inline_error :auth_token, "Cannot be blank"
        are_required_fields_present = false
      end
