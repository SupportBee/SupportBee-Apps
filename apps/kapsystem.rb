...
module Kapsystem
  class Base < SupportBeeApp::Base
    string :username, :required => true, :label => 'Kapsystem Username', :hint => "Signup for a BulkSMS account at http://sms1.kapsystem.com/"
    #password :password, :required => true, :label => 'Kapsystem Password'
    #string :mobilenumber, :required => true, :label => 'Phone Number(s)', :hint => 'Ex: 44123123456,44123123457 (with country code, comma separated)'
  #string :senderid, :required => true, :label => 'Sender ID'
    ...
    def notify_sms(message)
      puts message
      http_post "http://trans.kapsystem.com/web2sms.php" do |req|
        req.params[:username] = settings.username
        req.params[:password] = settings.password
        req.params[:message] =  message[0...160]
        req.params[:mobilenumber] = settings.mobilenumber 
		req:params[:senderid] = settings.senderid
      end
    end
