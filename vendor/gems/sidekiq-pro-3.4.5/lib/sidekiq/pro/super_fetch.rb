  ...
  class SuperFetch
    ...
    def self.bulk_requeue(in_progress, options)
      ...
      Sidekiq.redis do |conn|
        ...
        id = options[:identity]
        Sidekiq.logger.debug { "Unregistering super process #{id}" }
        conn.multi do
      ...
      Sidekiq.logger.warn { "Failed to requeue: #{ex.message}" }
    end

    def self.private_queue(q, options)
      "queue:sq_#{options[:identity]}_#{q}"
    end
    ...
    def self.check_for_orphans
      ...
      Sidekiq.redis do |conn|
        ...
        conn.scan_each(:match => "queue:sq_*", :count => 100) do |que|
          qcount += 1
          ...
          if !conn.sismember("super_processes", id)
            tq = name.join("_")
            qs << tq
            while conn.rpoplpush(que, "queue:#{tq}")
              orphans += 1
            end
          end
    ...
    class Retriever
      ...
      def cleanup_the_dead
        ...
        Sidekiq.redis do |conn|
          conn.sscan_each("super_processes") do |x|
            ...
            Sidekiq.logger.debug { "Cleaning up old super process #{x}" }
            ...
            qs.each do |priv|
              (_, _, *q) = priv.split("_")
              tq = q.join("_")
              while conn.rpoplpush(priv, "queue:#{tq}")
                count += 1
              end
            end
      ...
      def register_myself
        Sidekiq.redis do |conn|
          ...
          Sidekiq.logger.debug { "Registering super process #{identity} with #{qs}" }
          conn.multi do
